...
# percolator. Copied from stacks/silo/generate-secrets.ps1 2026-09-01 when
# percolator's stack directory was first scaffolded (see the runbook's
...
# nodes"). Fills in real values for percolator's app secrets, encrypts each
# with the age PUBLIC key (via .sops.yaml), and leaves committable
# ciphertext at secrets/percolator/<app>.sops.yaml. git add/commit/push those
# from here; percolator then decrypts its own copy at deploy time
# (stacks/percolator/decrypt-secrets.sh, needs the PRIVATE key).
#
# THIS FILE HAS NO APPS YET. percolator's own app list isn't built out —
# see stacks/percolator/README.md and the initiation doc for what's actually
...
$SecretsDir = Join-Path $RepoRoot "secrets\percolator"
...
Write-Host "==> percolator secrets (secrets/percolator/*.sops.yaml)"
...
# yet: percolator has no apps built out as of 2026-09-01.
#
...
Write-Host "`nDone. Encrypted files under secrets/percolator/ are ciphertext — safe to 'git add', commit, and push."
Write-Host "Next, on percolator: git pull, then ./decrypt-secrets.sh, then ./render-configs.sh."
