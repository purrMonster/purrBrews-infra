...
# automated fill-in on silo the way sieve's generate-secrets.sh handles its
# own `.env.local`'s DOMAIN — silo's README just said "fill in by hand,"
...
#      secrets/silo/*.sops.yaml into each app's secrets.env.local.
...
#      secrets/silo/*.sops.yaml on every decrypt, so patching it locally
