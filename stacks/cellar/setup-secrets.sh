...
# automated fill-in on cellar the way sieve's generate-secrets.sh handles its
# own `.env.local`'s DOMAIN — cellar's README just said "fill in by hand,"
...
#      secrets/cellar/*.sops.yaml into each app's secrets.env.local.
...
#      secrets/cellar/*.sops.yaml on every decrypt, so patching it locally
