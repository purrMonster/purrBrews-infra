...
# this exact gap: `.env.local` had no automated fill-in on cellar the way
# sieve's generate-secrets.sh handles its own `.env.local`'s DOMAIN — cellar's
...
#      make up on its own (an external API token, say) — none of cellar's
