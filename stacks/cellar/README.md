...
Deploy order, as best understood right now: sieve → silo → cellar (this
one) → ... — the exact position of cellar relative to the other
...
**not confirmed against the initiation doc's own section for cellar**.
...
cellar's actual app list is known (check the initiation doc's relevant
...
- `decrypt-secrets.sh` — the node-side half of the SOPS+age bridge.
  Decrypts `secrets/cellar/*.sops.yaml` into each app's
  `secrets.env.local`. Needs the age private key at
  `/etc/purrbrews/age/keys.txt` on cellar itself (see
  secrets/README.md) — copied from silo's version, `SECRETS_SRC`
  repointed at `secrets/cellar`.
- `generate-secrets.ps1` — the roastery-side half. Run this on roastery to
  add cellar's secrets to `secrets/cellar/*.sops.yaml` as apps get
