...
# mochaPot. Copied from stacks/silo/generate-secrets.ps1 2026-09-01 when
# mochaPot's stack directory was first scaffolded (see the runbook's
...
# nodes"). Fills in real values for mochaPot's app secrets, encrypts each
# with the age PUBLIC key (via .sops.yaml), and leaves committable
# ciphertext at secrets/mochaPot/<app>.sops.yaml. git add/commit/push those
# from here; mochaPot then decrypts its own copy at deploy time
# (stacks/mochaPot/decrypt-secrets.sh, needs the PRIVATE key).
#
# THIS FILE HAS NO APPS YET. mochaPot's own app list isn't built out —
# see stacks/mochaPot/README.md and the initiation doc for what's actually
...
$SecretsDir = Join-Path $RepoRoot "secrets\mochaPot"
...
Write-Host "==> mochaPot secrets (secrets/mochaPot/*.sops.yaml)"
...
# yet: mochaPot has no apps built out as of 2026-09-01.
#
...
Write-Host "`nDone. Encrypted files under secrets/mochaPot/ are ciphertext — safe to 'git add', commit, and push."
Write-Host "Next, on mochaPot: git pull, then ./decrypt-secrets.sh, then ./render-configs.sh."
