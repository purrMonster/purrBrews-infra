...
# automated fill-in on percolator the way sieve's generate-secrets.sh handles its
# own `.env.local`'s DOMAIN — percolator's README just said "fill in by hand,"
...
#      secrets/percolator/*.sops.yaml into each app's secrets.env.local.
...
#      secrets/percolator/*.sops.yaml on every decrypt, so patching it locally
