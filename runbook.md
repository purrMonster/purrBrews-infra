...
- [ ] Run `./pihole-dns-bootstrap.sh` for real on sieve at least once — the split-horizon fix it formalizes was already applied live via raw `pihole-FTL --config` commands during the 2026-08-30 login-test troubleshooting (and confirmed working), but the script itself hasn't been exercised yet. Confirm it correctly reports "already present, nothing to do" against that live state (proves the idempotency check works), not just that a fresh run can create entries from empty.
