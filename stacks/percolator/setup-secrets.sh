...
# this exact gap: `.env.local` had no automated fill-in on percolator the way
# sieve's generate-secrets.sh handles its own `.env.local`'s DOMAIN — percolator's
...
#   3. Runs ./generate-secrets.sh — generates every secret percolator's apps
#      need, locally, right here on percolator, straight into each app's own
...
#      make up on its own (an external API token, say) — none of percolator's
