...
- [ ] Authelia OIDC rollout (2026-09-06 entry) — generate the JWKS key on sieve (`openssl genpkey`), run `sieve/generate-secrets.sh`, paste each app's plaintext client secret into its own node per the printed table, then `render-configs.sh` + recreate `authelia` on sieve. Nothing works until this happens.
