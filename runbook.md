...
- [ ] Verify Unbound is actually restricted to sieve, not just configured to be — `dig @<SILO_LAN_IP> example.com` from sieve should answer; the same from any other LAN device should come back `REFUSED` or time out. Do this before it carries any real traffic.
