...
# cellar. Copied from stacks/silo/generate-secrets.ps1 2026-09-01 when
# cellar's stack directory was first scaffolded (see the runbook's
...
# nodes"). Fills in real values for cellar's app secrets, encrypts each
# with the age PUBLIC key (via .sops.yaml), and leaves committable
# ciphertext at secrets/cellar/<app>.sops.yaml. git add/commit/push those
# from here; cellar then decrypts its own copy at deploy time
# (stacks/cellar/decrypt-secrets.sh, needs the PRIVATE key).
#
# THIS FILE HAS NO APPS YET. cellar's own app list isn't built out —
# see stacks/cellar/README.md and the initiation doc for what's actually
...
$SecretsDir = Join-Path $RepoRoot "secrets\cellar"
...
Write-Host "==> cellar secrets (secrets/cellar/*.sops.yaml)"
...
# yet: cellar has no apps built out as of 2026-09-01.
#
...
Write-Host "`nDone. Encrypted files under secrets/cellar/ are ciphertext — safe to 'git add', commit, and push."
Write-Host "Next, on cellar: git pull, then ./decrypt-secrets.sh, then ./render-configs.sh."
