...
- [ ] Verify Unbound is actually restricted to sieve — two attempts so far, both failed for real (`dig` from a Mac, then from percolator, both got `NOERROR` instead of `REFUSED`), both root-caused and fixed the same day (see the two 2026-08-31 Unbound access-control entries below). Third `dig` re-test, after the second fix is deployed, is what needs to actually pass before this gets checked off.
...
Not yet done: deploying and re-testing this second fix (commit/push/pull/render/restart on silo, then `dig` from a non-sieve device again — third time this specific check has needed to actually pass before it's trusted).
