...
#
# Decrypts every secrets/mochaPot/<app>.sops.yaml (committed ciphertext,
...
#
# Run this after every `git pull` that touches secrets/mochaPot/, and always
...
# source of truth (secrets/mochaPot/*.sops.yaml) every time. Safe to re-run
...
SECRETS_SRC="${REPO_ROOT}/secrets/mochaPot"
...
if [[ ! -d "$SECRETS_SRC" ]]; then
  log "No ${SECRETS_SRC} yet — nothing to decrypt (fine if no mochaPot app needs secrets yet)."
  exit 0
fi
...
log "Decrypting ${#FILES[@]} secret file(s) from secrets/mochaPot/"
for f in "${FILES[@]}"; do
  app="$(basename "$f" .sops.yaml)"
  out_dir="${DIR}/${app}"
  if [[ ! -d "$out_dir" ]]; then
    echo "  SKIP ${app}: ${out_dir} doesn't exist — no such app folder under stacks/mochaPot. Check the filename matches the app directory name." >&2
    continue
  fi
  ...
  mv "${out}.tmp" "$out"
  chmod 600 "$out"
  echo "  decrypted: secrets/mochaPot/${app}.sops.yaml -> stacks/mochaPot/${app}/secrets.env.local"
done
