...
Stack directory scaffolded 2026-09-01, ahead of mochaPot actually existing
...
Deploy order, as best understood right now: sieve → silo → mochaPot (this
one) → ... — the exact position of mochaPot relative to the other
...
**not confirmed against the initiation doc's own section for mochaPot**.
...
mochaPot's actual app list is known (check the initiation doc's relevant
...
- `setup-secrets.sh` — first-time-setup script: creates `.env.local` from
  ...
  its silo-specific prose adjusted to mochaPot.
- `decrypt-secrets.sh` — the node-side half of the SOPS+age bridge.
  Decrypts `secrets/mochaPot/*.sops.yaml` into each app's
  `secrets.env.local`. Needs the age private key at
  `/etc/purrbrews/age/keys.txt` on mochaPot itself (see
  secrets/README.md) — copied from silo's version, `SECRETS_SRC`
  repointed at `secrets/mochaPot`.
- `generate-secrets.ps1` — the roastery-side half. Run this on roastery to
  add mochaPot's secrets to `secrets/mochaPot/*.sops.yaml` as apps get
...
- `local.env.example` — copy to `.env.local` and fill in. Minimal for now
  (`MOCHAPOT_LAN_IP`, `TZ`, `DOMAIN`) — grows as apps are added, same
  as silo's did.
...
Once mochaPot exists and has at least one app built:
...
- The deploy-order guess above (mochaPot positioned after silo, before/after
...
- If mochaPot will run a Komodo Periphery agent (so silo's Komodo Core can
  ...
  — copy it into an app subdirectory here once mochaPot exists and fill in
