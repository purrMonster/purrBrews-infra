...
# this exact gap: `.env.local` had no automated fill-in on ristretto the way
# sieve's generate-secrets.sh handles its own `.env.local`'s DOMAIN — ristretto's
...
#   3. Runs ./generate-secrets.sh — generates every secret ristretto's apps
#      need, locally, right here on ristretto, straight into each app's own
...
#      make up on its own (an external API token, say) — none of ristretto's
