...
- [ ] Run `stacks/silo/generate-secrets.ps1` and `stacks/silo/decrypt-secrets.sh` for real, once silo's first app that needs a secret is actually built (Komodo, most likely) — currently untested against a real encrypted file, only reasoned through (see 2026-08-31 entry)
