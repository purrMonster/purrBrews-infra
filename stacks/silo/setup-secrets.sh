...
# this exact gap: `.env.local` had no automated fill-in on silo the way
# sieve's generate-secrets.sh handles its own `.env.local`'s DOMAIN — silo's
...
#   3. Runs ./generate-secrets.sh — generates every secret silo's apps
#      need, locally, right here on silo, straight into each app's own
...
#      make up on its own (an external API token, say) — none of silo's
