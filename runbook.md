...
- [x] Authelia OIDC rollout, foundation + secret handoff — done 2026-09-06. JWKS key generated on sieve (hit and fixed a real config bug along the way, see that dated entry — `key_path` isn't a real Authelia field), `identity_providers.oidc` confirmed live (`/.well-known/openid-configuration` returns a real discovery doc). Secret handoff automated via new `oidc-secret-sync.sh` (SSH push, value piped through stdin only, never on a command line) instead of manual copy-paste — all 10 cross-node apps + Komodo/Vaultwarden/etc. confirmed distributed and verified by User Penguin.
- [ ] Once secrets are pasted in: test each app's OIDC login for real, one at a time — expect `invalid_client`/redirect-mismatch debugging, not a clean first pass, same as every other first-bring-up in this project. Start with the ones flagged "confirmed" (Mealie, FreshRSS, Actual Budget, Komodo, Headscale) before the "unconfirmed literal names" ones (Vikunja, Vaultwarden) or the manual-step ones (Nextcloud, Home Assistant, Jellyfin, Immich).
- [ ] Decide whether to pursue the unofficial n8n reverse-proxy/oauth2-proxy SSO workaround, or leave n8n on native login permanently (2026-09-06 entry)
- [ ] Re-verify Nextcloud/Paperless-ngx/Home Assistant/Vaultwarden's exact OIDC redirect_uri paths against current primary sources before first login attempt — these four were reconstructed from an earlier research pass, not fresh-verified 2026-09-06 (see that entry)
...
## 2026-09-06 — Authelia as OpenID Connect Provider, fleet-wide SSO rollout ("everything in one pass")

User Penguin: "let's get OIDC setup for all containers so that they can login using authelia." Scoping question answered "Everything in one pass" — build the Authelia OIDC foundation AND every app's client config in one session, accepting that some apps need manual follow-up (Nextcloud's app install, Home Assistant's HACS component, Jellyfin's plugin) done later by the user, rather than doing the foundation first and coming back per-app later.

Research done via 5 parallel subagents across two sessions (three were cut off by a platform rate limit partway through and had to be re-run fresh once it reset — no state carried over, re-researched from scratch, not resumed). Covered all ~15 apps in this fleet that plausibly speak OIDC: Vikunja, Mealie, Immich, FreshRSS, Actual Budget, Nextcloud, Paperless-ngx, Home Assistant, Vaultwarden, Komodo, Headscale, Jellyfin, n8n, Stirling PDF, Roundcube.

**Three apps excluded from the rollout, each for a different real reason, not an oversight:**
- **n8n** — SSO/OIDC is a Business/Enterprise-license-only feature per n8n's own docs, not present in the free `n8nio/n8n` Community image this fleet runs. An unofficial nginx+oauth2-proxy workaround exists (linked in `configuration.yml.template`'s comment) but wasn't decided on.
- **Stirling PDF** — confirmed OAuth2/OIDC moved behind a paid "Team" tier ($99/mo) as of the v1.0.0+ open-core relicense. Authelia's own published Stirling-PDF integration guide is stale on this (tested against the pre-relicense v0.42.0) — a real trap if followed blindly.
- **Roundcube** — this fleet has no mail server of its own; Roundcube points at a real external IMAP/SMTP provider (Gmail/Fastmail). Roundcube's OAuth2 support is *delegated* auth — the same OIDC token used for webapp login also gets handed to IMAP/SMTP via XOAUTH2, which only works if the mail backend trusts tokens from the same IdP. Gmail/Fastmail will never accept an Authelia-issued token, so enabling this would only break real mail fetching while replacing a login screen nobody needed replaced. Caught by reasoning about this fleet's actual architecture, not by anything in Roundcube's own docs.

**Foundation, built on sieve** (`stacks/sieve/authelia/`):
- `identity_providers.oidc` block added to `configuration.yml.template` — `hmac_secret` (random, via generate-secrets.sh), JWKS key via `key_path` (file-based, not embedded inline — a PEM's own newlines don't survive envsubst + YAML block-scalar indentation cleanly, confirmed by reasoning through the render pipeline before writing anything, not by hitting the bug live), and one `clients:` entry per included app.
- **Every client's `authorization_policy` is `one_factor`**, matching this file's own existing `access_control` policy — a real catch made before writing any client block: Authelia's own example configs default to `two_factor`, and copying that blindly would have made every app's OIDC login permanently fail for everyone, since nobody has TOTP enrolled yet on this fleet. Flagged in the file's own header comment for whoever tightens `access_control` later to remember to also flip these.
- **New `generate-secrets.sh` gotcha, caught before it caused a real bug**: PBKDF2 client-secret hashes contain literal `$` characters (`$pbkdf2-sha512$19$salt$hash`). This repo's `render-configs.sh` renders `.template` files by `source`-ing each app's `secrets.env.local` (bash, interprets `$` as variable expansion) and then running `envsubst`. An unquoted hash value would get silently mangled by bash's own `source` — `$pbkdf2` parses as a reference to an unset shell variable and vanishes. Fixed by single-quoting every `_HASH` value when writing it (`printf "%s='%s'\n"`, not the existing unquoted `set_if_absent`) — this specific quoting requirement didn't exist anywhere else in this repo's secrets before today, since no prior secret ever contained a `$`.
- **Cross-node secret handoff — new problem, no existing mechanism covered it**: Authelia (sieve) needs a client's secret HASH; the app itself (almost always a different physical node) needs the PLAINTEXT. No SSH access between nodes exists in this operating model. Solved by having sieve's `generate-secrets.sh` generate both, keep the hash, and print a copy-paste table (app / node / file / KEY=VALUE) for the plaintext half — the user pastes each value into that app's own node's `secrets.env.local` by hand, then re-runs that node's own script. Headscale is the one exception (same node as Authelia) — handled automatically, no copying needed.
- JWKS RSA key generation is a manual one-time `openssl genpkey` step on sieve — documented in `sieve/README.md`'s new "Authelia as an OpenID Connect Provider" section, which also carries the full one-time setup sequence and a per-app rollout-status table.

**Per-app work, one node at a time:**
- **mochaPot**: Vikunja (env vars — Vikunja's own OIDC doc only showed a `config.yml` example, not confirmed literal env var names, flagged), Mealie (env vars, confirmed), FreshRSS (env vars, confirmed — flagged a real lockout risk: FreshRSS derives the OIDC username from a claim and has no admin-recovery path if it doesn't match an existing account), Actual Budget (env vars, confirmed — flagged a real ownership risk: the FIRST OIDC login permanently becomes the server owner, no UI way to change it later), Immich (Admin-UI-only, no compose option exists — documented as a manual step, flagged that its role-claim mapping only applies at first account creation, never re-synced).
- **percolator**: Home Assistant (HACS `hass-oidc-auth` component, alpha-stage per its own maintainer — documented as manual, local login kept as real fallback), Nextcloud (`occ` commands + `user_oidc` app install, no compose option — documented with the exact commands, flagged `client_secret_post` is specific to `user_oidc` and NOT the same as the alternative `oidc_login` app), Paperless-ngx (env vars, `PAPERLESS_SOCIALACCOUNT_PROVIDERS` JSON reconstructed from an earlier, pre-compaction research pass rather than fresh-verified this pass — flagged to double-check against paperless-ngx's current docs before trusting the JSON schema).
- **cellar**: Vaultwarden (`SSO_*` env vars — flagged not independently re-verified this pass, confirm against Authelia's own Vaultwarden guide; also flagged Vaultwarden's SSO feature needs >=1.35.0 and this stack pins `:latest`, so the actual running version needs a manual check, not an assumption).
- **silo**: Komodo (env vars, confirmed — also updated `KOMODO_HOST` from `http://` to `https://` for consistency now that this value gets echoed into an OIDC redirect; noted Komodo already sits behind this same Authelia's ForwardAuth on this node, so the OIDC client is complementary SSO into Komodo's own login, not a redundant second gate; Komodo has no group/role mapping at all yet — open upstream feature request — so new OIDC users still need manual enabling).
- **roastery**: Jellyfin (plugin-UI-only — flagged heavily: the only maintained SSO plugin, `9p4/jellyfin-plugin-sso`, was archived by its own author 2026-05-12, no fork named anywhere; also flagged Jellyfin here has no DOMAIN/HTTPS front end at all, LAN-only plain HTTP, so the placeholder redirect URI in Authelia's client config needs the real reachable address substituted in before use).

Every compose file touched was YAML-validated (dummy-substituting `${VAR}` placeholders, then `yaml.safe_load`) before being called done, and every shell script touched was `bash -n`-checked.

### Still open

- Every `_FROM_SIEVE_AUTHELIA` placeholder secret across mochaPot/percolator/cellar/silo/roastery still needs the real value pasted in from sieve's `generate-secrets.sh` output — nothing above actually works end to end until that handoff happens.
- The JWKS key hasn't been generated yet — that's the literal first step, blocking everything else, and has to be run by the user on sieve.
- Four apps' redirect_uris (Nextcloud, Paperless-ngx, Home Assistant, Vaultwarden) are carried over from an earlier, pre-compaction research pass rather than fresh-verified this session — see `configuration.yml.template`'s own confidence note. Worth a second look before the first real login attempt on each.
...
## 2026-09-07 — Root cause of the Mealie OIDC "client secret did not match" saga, plus a full pre-reinit repass

User Penguin spent hours re-verifying Mealie's OIDC client secret against Authelia (hash validation, cross-node plaintext checksum, the live rendered config) — every check came back matching, yet `docker logs mealie` kept showing `invalid_client: Client authentication failed` on every login attempt, unchanged across many "successful" `docker compose up -d --force-recreate authelia` cycles. The secret itself was never wrong at any point.

**Actual root cause, found by reading a full (not grep-filtered) `render-configs.sh` run**: `stacks/sieve/generate-secrets.sh`'s `hash_oidc_secret()` called plain `docker run` (no `sudo`). `purrbrews-init.sh` deliberately keeps `$OPS_USER` (barista) out of the docker group (see its own comment on `OPS_USER`), so plain `docker run` always failed for barista — the only way anyone had gotten it working before was running the *whole* `generate-secrets.sh` as `sudo`, which left every file it touched that run root-owned, including apps needing zero privilege (`headscale/secrets.env.local` among them). The next plain (non-sudo) `render-configs.sh` run then hit `Permission denied` sourcing that root-owned file and, under `set -euo pipefail` inside a `find | while read` pipeline (a subshell in bash), silently aborted the *entire* render loop before ever reaching `authelia/config/configuration.yml.template` later in `find`'s ordering. Authelia kept serving a stale rendered config through every "successful" recreate — the container restarted fine, it just never picked up the real client secret hash.

Two independent fixes, both tested:
- `generate-secrets.sh`'s `hash_oidc_secret()` now runs `sudo docker run` for just that one line, so the script never needs to run as root as a whole and never poisons file ownership again.
- `render-configs.sh` (all 7 nodes) no longer aborts the whole loop on one template's failure — `set -e` dropped, the `find | while read` subshell replaced with `done < <(find ...)` (so a `FAILED` array actually persists), and it now reports every failure at the end and exits non-zero only then. Already-rendered files from earlier in the loop are untouched by a partial failure.

Along the way, two of my own diagnostic mistakes cost real time and are worth remembering as a class, not just this instance: a `grep -i client_secret` filter missed Authelia's real error line because the log text says "client secret" (a space, not an underscore) — always read the full unfiltered log before concluding a service isn't even processing a request. And a `grep -A20` window bled into the *next* OIDC client's `client_secret:` line in `configuration.yml` (each client block is ~15 lines), producing a false "mismatch" — fixed with an `awk` extraction that stops at the first match after the right `client_id:` line.

**Pre-reinit repass** (User Penguin: "do a repass" while away from the keyboard, following the decision to OS-reinit sieve/silo/cellar/percolator/mochaPot this week — roastery decoupled as its own track, ristretto parked as dead hardware). Read every remaining sieve bootstrap script, every node's `generate-secrets.sh`/`setup-secrets.sh`, cross-checked every `${VAR}` reference in every `docker-compose.yml`/`*.template` against what each node's own `generate-secrets.sh`/`local.env.example` actually defines, and read silo's/percolator's/roastery's full READMEs. Found and fixed:

- `initialization/purrbrews-init.sh`'s base apt package list was missing `jq` — four of sieve's own bootstrap scripts (`crowdsec-bouncer-bootstrap.sh`, `headscale-bootstrap.sh`, `lldap-bootstrap.sh`, `pihole-dns-bootstrap.sh`) require it and would have failed immediately on a freshly reinitialized sieve.
- `stacks/percolator/generate-secrets.sh`'s header still said "THIS FILE HAS NO APPS YET," left over from percolator's 2026-09-02 scaffold — stale since 2026-09-04/05 once homeassistant/nextcloud/paperless/traefik/komodo-periphery were actually added. Corrected.
- **Real bug, not just stale prose**: `stacks/mochaPot/local.env.example` never had a `SIEVE_LAN_IP` line, even though `mochaPot/traefik/config/dynamic.yml.template` has needed it since Traefik was added there 2026-09-06 (ForwardAuth's direct hop to sieve's Authelia, same pattern as silo's and percolator's own Traefik). `render-configs.sh`'s `envsubst` doesn't error on an unset variable, it just substitutes an empty string — a fresh mochaPot reinit would have silently rendered `http://:9091/api/authz/forward-auth` with no error at any step, breaking SSO gating for every Traefik-fronted app on that node. The live node's real `.env.local` presumably has this added by hand already (Traefik/SSO have been working); only the example file that a fresh reinit actually copies was missing it. Added, matching silo's/percolator's existing pattern.
- `stacks/percolator/README.md`'s and `stacks/mochaPot/README.md`'s "What's here now" descriptions of their own `local.env.example` files had drifted — both listed only the original 2026-09-01 scaffold vars, not the several added since. Corrected to point at the file itself as the source of truth rather than a summary that can drift again.

Everything else checked this pass came back clean: `headscale-bootstrap.sh`/`pihole-dns-bootstrap.sh`/`crowdsec-bouncer-bootstrap.sh` all already self-detect `docker` vs `sudo docker` correctly and are idempotent; every other node's `generate-secrets.sh` has no `sudo`/`docker` calls at all (so the ownership-poisoning bug class can't recur there); the entire OIDC secret pipeline (sieve's `set_oidc_client` calls → its printed handoff table → every other node's `_OIDC_CLIENT_SECRET` keys → `authelia/config/configuration.yml.template`'s `client_id`/`client_secret` entries → each app's own `docker-compose.yml` env var) cross-checked consistent end to end for all 11 apps plus Headscale.

### Still open

- None of this has been exercised on a real reinit yet — this is all repo-level review, no fleet node has actually been wiped/reimaged this week.
- `REINIT_PLAN.md` (repo root) has the actual per-node sequence for this week's work; not repeated here.
- Same fleet-wide ufw/Docker-NAT-bypass gap flagged in `stacks/silo/README.md`'s Known gaps (2026-09-04 entry above) is still unresolved everywhere except silo's three de-published apps — worth actually fixing during the reinit rather than carrying forward again.
