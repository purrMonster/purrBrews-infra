...
#
# Decrypts every secrets/cellar/<app>.sops.yaml (committed ciphertext,
...
#
# Run this after every `git pull` that touches secrets/cellar/, and always
...
# source of truth (secrets/cellar/*.sops.yaml) every time. Safe to re-run
...
SECRETS_SRC="${REPO_ROOT}/secrets/cellar"
...
if [[ ! -d "$SECRETS_SRC" ]]; then
  log "No ${SECRETS_SRC} yet — nothing to decrypt (fine if no cellar app needs secrets yet)."
  exit 0
fi
...
log "Decrypting ${#FILES[@]} secret file(s) from secrets/cellar/"
for f in "${FILES[@]}"; do
  app="$(basename "$f" .sops.yaml)"
  out_dir="${DIR}/${app}"
  if [[ ! -d "$out_dir" ]]; then
    echo "  SKIP ${app}: ${out_dir} doesn't exist — no such app folder under stacks/cellar. Check the filename matches the app directory name." >&2
    continue
  fi
  ...
  mv "${out}.tmp" "$out"
  chmod 600 "$out"
  echo "  decrypted: secrets/cellar/${app}.sops.yaml -> stacks/cellar/${app}/secrets.env.local"
done
